...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    D3DX11CreateShaderResourceViewFromMemory(var->winapi.device_dx11, menu_background, sizeof(menu_background), &info, pump, &var->gui.menu_background, 0);
    for (int i = 0; i < IM_ARRAYSIZE(img_for_versions); i++)
        D3DX11CreateShaderResourceViewFromMemory(var->winapi.device_dx11, decoration[i], sizeof(decoration[i]), &info, pump, &var->gui.decoration[i], 0);

    for (int i = 0; i < IM_ARRAYSIZE(img_for_versions); i++)
        D3DX11CreateShaderResourceViewFromMemory(var->winapi.device_dx11, img_for_versions[i], sizeof(img_for_versions[i]), &info, pump, &var->gui.img_for_versions[i], 0);
    ...
    for (int i = 0; i < IM_ARRAYSIZE(flags); i++)
        D3DX11CreateShaderResourceViewFromMemory(var->winapi.device_dx11, flags[i], sizeof(flags[i]), &info, pump, &var->gui.flags[i], 0);

    var->gui.cs_player.g_pd3dDevice = var->winapi.device_dx11;
    var->gui.cs_player.g_pd3dDeviceContext = var->winapi.device_context;
    var->gui.apex_player.g_pd3dDevice = var->winapi.device_dx11;
    var->gui.apex_player.g_pd3dDeviceContext = var->winapi.device_context;
    var->gui.fortnite_player.g_pd3dDevice = var->winapi.device_dx11;
    var->gui.fortnite_player.g_pd3dDeviceContext = var->winapi.device_context;

    var->gui.cs_player.init_video("cs_demo.mp4");
    var->gui.apex_player.init_video("apex_demo.mp4");
    var->gui.fortnite_player.init_video("fortnite_demo.mp4");
    ...
    while (!done)
    {
        ...
        //HRESULT hr = var->winapi.swap_chain->Present(0, 0); // Present without vsync
        g_SwapChainOccluded = (hr == DXGI_STATUS_OCCLUDED);
    }

    var->gui.cs_player.cleanup_video();
